  ...
  return (
    <Row className="h-screen">
      <Col span={5} style={{ boxShadow: '0px 0px 2px rgba(0,0,0,.2)' }}>
        <TabItem text="Friend Lists" icon={<IoPersonOutline size={22} />} active />
        <TabItem text="Joined Groups" icon={<IoPeopleOutline size={24} />} />
        <TabItem text="Friend Requests" icon={<IoPersonAddOutline size={22} />} />
      </Col>
      <Col span={19} >
...
const TabItem = ({ text, icon, active }) => {
  return (
    <Flex className={`${active ? 'bg-blue-50 ' : ''} p-4 cursor-pointer hover:bg-gray-100`} gap={20} align="center">
      {icon}
